...
* 部分列写入必须包含主键字段；分区字段可以由 `column` 提供，也可以在 `OVERWRITE_PARTITION` 模式下由 `write.overwritePartition.partition` 提供。
...
* `sink.options` 是 Paimon 建表属性扩展区，例如 `bucket`、`file.format` 等。
...
`OVERWRITE_PARTITION` 用于覆盖指定分区。为了保证分区覆盖原子性，该模式在任务结束时单次 commit，不按 `write.batchSize` 多次 commit。
...
上例中 `dt` 分区字段未出现在 `column` 中，执行器会使用 `write.overwritePartition.partition.dt` 作为写入行的分区值。
...
| `write.batchSize` | 每批 commit 的记录数。`OVERWRITE_PARTITION` 模式不按该值多次 commit。 | 可选 | `1000` |
...
| `sink.options` | Paimon 建表属性扩展区。 | 可选 | 无 |
...
当固定字段与动态 KV 同名时，以固定字段为准。`sink.options` 属于可扩展参数区，主要用于 Paimon 建表属性。
...
| --- | --- | --- | --- | --- |
| `APPEND` | 追加写入。 | 无主键表 | `INSERT` | 按 `write.batchSize` 分批 commit |
| `UPSERT` | 主键合并写入。 | 主键表 | `UPDATE_AFTER` | 按 `write.batchSize` 分批 commit |
